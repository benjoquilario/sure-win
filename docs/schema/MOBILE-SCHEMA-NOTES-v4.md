...
**Version 4 {M} 29 August 2026.** The current document. v1, v2 and v3 are kept
beside it; where any of them disagree, this one is right.

### What changed in v4

v3 answered the schema half of `BACKEND-REQUESTS.md`. v4 answers the review of
that reply {M} the two items round two raised, plus four things neither round had
asked for, one of which was a live personal-data leak.

**Sections 1{M}19 keep their numbers**, as they did across v2 and v3. v4 appends
sections 20{M}23 and edits some of the earlier ones in place.

#### Three breaking changes. Read these before anything else.

| | What broke | What to do |
| --- | --- | --- |
| **1** | `user_profiles` is no longer readable by other members. Rendering somebody else's name or avatar from it now returns nothing. | Read `user_public_profiles` instead {M} [section 20](#20-privacy-what-each-member-can-see) |
| **2** | `likesCount` on `posts`, `comments` and `replies` is read-only. Writing it is now rejected. | Write only the like row; display the count as described in [section 21](#21-community-in-v4) |
| **3** | Community rows can be hidden rather than deleted. A feed that does not filter will show removed posts. | Add `Query.equal('isDeleted', false)` to every community read {M} [section 21](#21-community-in-v4) |

Signing up now writes **two** rows, not one: `user_profiles` and
`user_public_profiles`. A member without the second has no byline in the forum.

#### Everything else

| | |
| --- | --- |
| **Closed: the `member_shared` hole** | `posts`, `comments` and `replies` are `member_public`. No member can edit another member's words any more. Round two put this above #7 and arguably above #3; it is done, and it needed no Function. |
| **Closed: `user_profiles` leaked PII** | `email`, `licenseNumber` and `schoolOrEmployer` were readable by every signed-in account. Not any more. |
| **New table** | `user_public_profiles` {M} the part of a member others may see |
| **New table** | `user_blocks` {M} [section 21](#21-community-in-v4) |
| **New table** | `announcement_reads` {M} an unread badge that survives a reinstall |
| **New section 20** | [Privacy {M} what each member can see](#20-privacy-what-each-member-can-see) |
| **New section 21** | [Community in v4](#21-community-in-v4) |
| **New section 22** | [Deleting an account](#22-deleting-an-account) |
| **New section 23** | [Upgrading the app from v3](#23-upgrading-the-app-from-v3) {M} the checklist |

Nothing changed about questions, materials, sessions, answers, progress,
settings or billing. If your screen does not touch a member's identity or the
community, v4 does not affect it.
