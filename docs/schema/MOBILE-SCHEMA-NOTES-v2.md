...
**Version 2 — 29 August 2026.** This is the current document. v1 is kept
alongside it as `MOBILE-SCHEMA-NOTES.md` for reference; where the two disagree,
this one is right.

### What changed in v2

v1 described what the tables hold. It never described who Appwrite lets touch
them, because nothing did — table permissions were set by hand in the console
and lived nowhere else. Tables added after the last manual pass shipped with no
permissions at all, and the app got
`The current user is not authorized to perform the requested action.` on Start
a sitting and on Recent Activity, while the dashboard rendered both perfectly.

| | |
| --- | --- |
| **New section 11** | [Permissions](#11-permissions-why-a-write-can-come-back-401) — the create-time rule, and what every table allows |
| **New gotcha 1** | a create without `permissions` writes a row nobody can read |
| **Section 10** | every create example now shows `permissions`, not just `data` |
| **New helpers** | `ownedRowPermissions`, `tableNeedsRowPermissions`, `getTableAccessModel` |
| **Sections 11–13** | renumbered to 12–14 |

Nothing about columns, enums, queries or defaults changed. If you have v1 open,
those parts still hold — which is why it is still here rather than deleted. The
section numbers are the one thing that moved: **v1's sections 11, 12 and 13 are
v2's 12, 13 and 14.** A link to "section 13" written before today means
[Roles](#14-roles-are-the-dashboards-who-a-member-is-is-yours), now section 14.

**What this means for app code:** if you already pass `permissions` on your
creates, nothing needs to change — the rows you have written were correct all
along, and it was the tables underneath them that were not. If you do not,
gotcha 1 is the whole of the fix.
...
5. [Gotchas that will cost you a day](#5-gotchas-that-will-cost-you-a-day) — **read this one** — nine ways to lose a day
...
| --- | --- | --- |
| `member_private` | `user_answers`, `user_progress`, `user_settings`, `study_sessions`, `user_activity_log`, `learning_history`, `learning_achievements`, `user_daily_activity`, `user_weekly_reports` | create; read/update/delete **only their own rows** |
...
| `server_private` | `subscriptions` | nothing directly; the server grants each row to the member it concerns |
